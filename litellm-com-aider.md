# Manual de Instalação e Configuração — LiteLLM Gateway + LM Studio + Aider

**Ambiente:** Natural Tecnologia
**PC Servidor:** Windows 11 Pro — IP `10.150.0.69`
**Escopo:** Gateway LLM compartilhado em rede local (sem exposição à internet), com modelos Anthropic, Gemini e modelos open source locais.
...
4. [Parte 2 — Configuração do Notebook do Dev](#parte-2--configuração-do-notebook-do-dev)
   ...
   - 4.2 [Instalação do Aider](#42-instalação-do-aider)
   - 4.3 [Configuração do Aider para usar o gateway](#43-configuração-do-aider-para-usar-o-gateway)
   - 4.4 [Teste de conexão](#44-teste-de-conexão)
   - 4.5 [Uso prático no dia a dia](#45-uso-prático-no-dia-a-dia)
...
- Porta 4000 acessível **apenas na LAN** (firewall bloqueia outras origens)
...
- Acesso à mesma rede local
- Permissão para ler/escrever em `~/.aider/`
...
Esta seção é o que você entrega para cada desenvolvedor.

## 4.1 Instalação do Python

(O dev pode pular se já tiver Python 3.10+)

1. Baixe Python em https://www.python.org/downloads/
2. Execute o instalador marcando **"Add python.exe to PATH"**
3. Verifique:
   ```powershell
   python --version
   pip --version
...
O comando `aider-install` cria um ambiente isolado para o Aider, evitando conflitos com outros pacotes Python do sistema.
...
## 4.3 Configuração do Aider para usar o gateway

O Aider precisa saber:

- O **endereço** do gateway (`http://10.150.0.69:4000`)
- A **virtual key** que o dev recebeu
- Qual **modelo** usar por padrão

### Opção A — Configuração via arquivo (recomendado)

Crie o arquivo `~/.aider.conf.yml` (no Windows: `C:\Users\<seu-usuario>\.aider.conf.yml`):

```yaml
# Endpoint do gateway
openai-api-base: http://10.150.0.69:4000
openai-api-key: sk-suaVirtualKeyAqui...

# Modelo padrão (use o nome configurado no gateway)
model: openai/claude-sonnet

# Modelo "arquiteto" para tarefas complexas (opcional)
architect: true
editor-model: openai/gemini-2-5-flash
...
> 📝 **Importante sobre o prefixo `openai/`:** mesmo o modelo sendo Claude ou Gemini, você usa o prefixo `openai/` porque o Aider trata o gateway LiteLLM como um endpoint OpenAI-compatible. O LiteLLM se encarrega de rotear pra origem correta.

### Opção B — Variáveis de ambiente

```powershell
[Environment]::SetEnvironmentVariable("OPENAI_API_BASE", "http://10.150.0.69:4000", "User")
[Environment]::SetEnvironmentVariable("OPENAI_API_KEY", "sk-suaVirtualKeyAqui...", "User")
```

Use as variáveis no nível **de usuário** (não de sistema), já que cada dev tem sua própria chave.

### Opção C — Linha de comando

```powershell
aider --openai-api-base http://10.150.0.69:4000 `
      --openai-api-key sk-suaVirtualKey... `
      --model openai/claude-sonnet
```

## 4.4 Teste de conexão

Teste rápido de conectividade com o gateway:

```powershell
$headers = @{
  "Authorization" = "Bearer sk-suaVirtualKey..."
  "Content-Type" = "application/json"
}
$body = @{
  model = "claude-sonnet"
...
Se retornar uma resposta com `"OK"`, a conexão está funcionando. Caso contrário, veja a seção de troubleshooting.

## 4.5 Uso prático no dia a dia

### Iniciar Aider em um projeto
...
O Aider abre o REPL e você pode começar a editar código.
...
```
/model openai/claude-opus     # tarefa complexa
/model openai/gemini-2-5-flash # tarefa rápida e barata
/model openai/qwen-14b-local   # privacidade total
```

### Architect mode (planejamento + execução)

```
/architect
```

O modelo principal (`claude-opus`) planeja, e o `editor-model` (`gemini-2-5-flash`) implementa. Combina raciocínio caro com escrita barata.

### Verificar gasto

A virtual key tem budget. Se o dev quiser saber quanto já gastou:

```powershell
$headers = @{ "Authorization" = "Bearer sk-suaVirtualKey..." }
Invoke-RestMethod -Uri "http://10.150.0.69:4000/key/info" -Headers $headers
```

Retorna `spend` acumulado e `max_budget` definido.
...
3. Dev consegue dar ping no servidor?
   ```powershell
   # No notebook do dev
...
**Sintoma:** `{"error": "Authentication Error"}`

**Causas:**

- Virtual key digitada errada → regenerar
- Virtual key revogada → criar nova via `/key/generate`
- Tentando usar modelo que a chave não tem permissão → atualizar permissões da chave
...
- Configurar fallback no `config.yaml` (já feito) — o LiteLLM vai automaticamente para o próximo modelo da lista

### Modelo local não responde

**Sintoma:** `qwen-14b-local` retorna erro.
...
Adicione system prompt ao chamar:
...
- [ ] API keys da Anthropic e Gemini configuradas **apenas** como variáveis de sistema, **nunca** dentro do `config.yaml` em texto plano
...
- [ ] Interface de rede classificada como **Private** (não Public)
...
- [ ] Pelo menos 1 dev teste fez uma requisição com sucesso pela rede
